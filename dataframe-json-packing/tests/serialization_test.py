...
def test_df_to_dict():
    df = pd.DataFrame({"string":["a", "b", None], 
                       "int": [1,2,None], 
                       "float": [1.1, 2.2, None], 
    ...
    assert packed == {'columns': [{'name': 'string', 'type': 'string', 'values': ['a', 'b', None]}, {'name': 'int', 'type': 'int', 'values': [1, 2, None]}, {'name': 'float', 'type': 'float', 'values': [1.1, 2.2, None]}, {'name': 'd', 'type': 'date', 'values': [730120, 730518, None]}]}
