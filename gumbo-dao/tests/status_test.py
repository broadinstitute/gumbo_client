...
def test_get_screen_status():
    ...
    assert (
        status.get_screen_status(
            ...
            screener_qc="PASS",
            cds_qc="PASS",
            consortium_release_date=past_date,
        )
    ...
    assert (
        status.get_screen_status(
            status=None, screener_qc="FAIL", cds_qc=None, consortium_release_date=None
        )
        == status.Status.failed
    )
    # In Progress
    ...
    assert (
        status.get_screen_status(
            ...
            screener_qc="PASS",
            cds_qc="PASS",
            consortium_release_date=future_date,
        )
        == status.Status.in_progress
    )
    assert (
    ...
    assert (
        status.get_screen_status(
            status=None, screener_qc=None, cds_qc=None, consortium_release_date=None
        )
        == None
    )
...
def test_get_omics_status():
    ...
    # Complete
    assert (
        status.get_omics_status(
            profile_status="Complete",
            main_sequencing_id="SomeIdVal",
            blacklist=None,
            consortium_release_date=past_date,
        )
        == status.Status.complete
...
def test_get_screen_failure_details():
    ...
    actual = status.get_screen_failure_details(
        screener_qc=None,
        cds_qc=None,
        screener_status="Terminal Fail",
        substatus="04 - Pooled screen queue",
    )
    ...
    actual = status.get_screen_failure_details(
        screener_qc="FAIL - Fingerprinting",
        cds_qc=None,
    ...
    actual = status.get_screen_failure_details(
        screener_qc="FAIL - Some other reason",
        cds_qc=None,
        screener_status="Done",
        substatus="something",
    )
