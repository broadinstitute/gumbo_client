...
def connection(monkeypatch):
    ...
    connection.execute("CREATE TABLE bulk_update_log (username varchar(100), \"timestamp\" TIMESTAMP, tablename varchar(100), rows_updated integer, rows_deleted integer, rows_inserted integer, reason varchar(1000))")
...
def test_update_no_delete(connection, dao):
    ...
    dao.update("username", "sample", new_df, delete_missing_rows=False, reason="test_update_no_delete")
    df = dao.get("sample")
...
def test_update_with_delete(connection, dao):
    ...
    new_df = pd.DataFrame({"PK":["X"], "COLUMN2": [3]})
    dao.update("username", "sample", new_df, delete_missing_rows=True, reason="test_update_with_delete")
    df = dao.get("sample")
